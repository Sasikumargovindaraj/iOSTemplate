...
import UIKit
@objc protocol GetSubItemstProtocol{
   optional func subItemsfailedWItherror(error error:NSError)
   optional func subItemsfoundPaths(paths paths:NSMutableArray)
}
class GetSubItemsHandler: NSObject,NSXMLParserDelegate {
    var parser:NSXMLParser!
    var delegate:GetSubItemstProtocol?
...
//                    self.presentViewController(alert, animated: true, completion: nil)
    ...
    func parserDidEndDocument(parser: NSXMLParser){
      self.delegate?.subItemsfoundPaths!(paths: pathsArray)
    }
}
