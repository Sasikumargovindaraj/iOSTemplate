...
public class EGFloatingTextField: UITextField {
    ...
    let kDefaultInactiveColor = UIColor.whiteColor()
    let kDefaultActiveColor = UIColor.whiteColor()
    let kDefaultErrorColor = UIColor.redColor()
    let kDefaultLineHeight = CGFloat(22)
    let kDefaultLabelTextColor = UIColor.whiteColor()
    ...
    func checkLabelStatus(){
        if self.text?.characters.count > 0{
            ...
            toTransform = CATransform3DTranslate(toTransform, -(CGRectGetWidth(self.label.frame)*(1/4))+20, -CGRectGetHeight(self.label.frame)+20, 0)
